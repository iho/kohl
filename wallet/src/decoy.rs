...
pub fn sample_decoys(
	...
	while out.len() < need {
		...
		if total <= 0.0 || !total.is_finite() {
			...
			let remaining: Vec<usize> =
				(0..candidates.len()).filter(|i| !picked.contains(i)).collect();
		...
		let i = chosen.unwrap_or_else(|| {
			(0..candidates.len()).rev().find(|i| !picked.contains(i)).expect("candidates remain")
		});
		picked.insert(i);
		weights[i] = 0.0;
		let c = &candidates[i];
	...
	if out.len() < need {
		return Err(DecoyError::NotEnoughCandidates { need, have: out.len() });
	}
	Ok(out)
}
